...
PLACEHOLDER_ALPHA = 75


class Textbox(MouseListener, KeyboardListener):
    def __init__(self, parent, *rect_args,
                 text_str="", font: pygame.font.Font = None, placeholder="",
                 char_limit=100, adaptive_char_limit=True,
                 input_validator: Callable[[str], bool] = str.isprintable, input_converter: Callable[[str], str] = str,
        ...
        :param input_validator: A customizable `(str) -> str` function that immediately converts inputted characters.
        ...
        placeholder_align = {"middle": "ctr", "left": "ml", "right": "mr"}[text_align]
        self._placeholder = WidgetComponent(self, 0, 0, 0, 0, "px", placeholder_align, placeholder_align)
        self._placeholder.image = self._font.render(self._placeholder_str, True, DEFAULT_FG_COLOR)
        self._placeholder.image.set_alpha(0 if self._text_str else PLACEHOLDER_ALPHA)
    ...
    def key_down(self, event):
        ...
        if event.key == pygame.K_BACKSPACE:
            ...
            self._text_str += self._input_converter(c)
    ...
    def update(self, dt):
        ...
        if self._label_hybrid:
            ...
            if self.hover and not self._editing:
                to_underline: WidgetComponent = self._text if self._text_str else self._placeholder

                if self._underline.rect.w != to_underline.rect.w:
                    # Update the underline's size and position
                    self._underline.rect.w = to_underline.rect.w
                ...
                self._underline.set_pos(to_underline.rect.centerx, to_underline.rect.top + self._font.get_ascent() + 2,
                                       "px", "tl", "mt")
                self._underline.image.set_alpha(255 if self._text_str else PLACEHOLDER_ALPHA)
            else:
        ...
        if self._placeholder_str and self._prev_editing != self._editing:
            self._placeholder.fade_anim(0.1, 0 if self._editing or self._text_str else PLACEHOLDER_ALPHA)
